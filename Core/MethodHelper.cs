    ...
    public static class MethodHelper
    {
...
#else
        public static MethodInfo GetCapableMethod(Type type, Type[] typeArguments, string methodName, bool staticMethod, object[] arguments, out object[] oargs)
#endif
        {
            ...
            object[] args = (from t in arguments select !(t is TypedNull) ? t : null).ToArray();
                                                      ...
                                                      object[] args, out object[] oargs)
        {
            ...
            MethodBuffer retVal =
                methods.FirstOrDefault(
                    n => EqualSignatures(n.MethodInfo.GetParameters(), types) && n.MethodInfo.IsStatic == staticMethod);
            if (retVal != null)
            {
...
#else
                    if (MakeCapableMethodArguments(m.MethodInfo.GetParameters(), types, args, m.ArgumentsRaw))
#endif
                    {
                ...
                if (parameters.Length != 0)
                {
                    ...
                    if (Attribute.IsDefined(last, typeof (ParamArrayAttribute)))
                    {
                        ...
                        if (paramArray.Length != 0)
                        {
                            try
                            {
                                Array.Copy(arguments, parameters.Length-1,
                                           paramArray, 0,
                                           paramArray.Length);
                            }
                            catch (Exception ex)
...
#else
                                          MethodBuffer mi = new MethodBuffer { MethodInfo = n, IsGeneric = n.ContainsGenericParameters, ArgumentsRaw = new object[n.GetParameters().Length] };
#endif
        ...
        private static bool EqualSignatures(ParameterInfo[] parameters, Type[] parameterTypes)
        {
            ...
            if (retVal)
            {
                ...
                {
                    //bool rv = retVal;
                    retVal &= parameters[i].ParameterType.IsAssignableFrom(parameterTypes[i]);
                    /*if (rv && !retVal)
                    {
